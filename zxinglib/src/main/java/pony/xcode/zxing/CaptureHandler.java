  ...
  /*  private boolean isScreenPortrait(Context context) {
    ...
    private ResultPoint transform(ResultPoint originPoint) {
        ...
        if (screenPoint.x < screenPoint.y) {
            ...
            x = originPoint.getX() * scaleX - Math.max(screenPoint.x, cameraPoint.y) / 2f;
            y = originPoint.getY() * scaleY - Math.min(screenPoint.y, cameraPoint.x) / 2f;
        } else {
            scaleX = 1.0f * screenPoint.x / cameraPoint.x;
            scaleY = 1.0f * screenPoint.y / cameraPoint.y;

            x = originPoint.getX() * scaleX - Math.min(screenPoint.y, cameraPoint.y) / 2f;
            y = originPoint.getY() * scaleY - Math.max(screenPoint.x, cameraPoint.x) / 2f;
        }
